import { i } from './Popover-Cvrg36vf.mjs';
import { v as Wt$1, s as pm, _ as Rb, u as da, t as Ky, y as Xm, z as Ym, P as _m } from './server.mjs';
import { m as m$1 } from './index-CVudU8ow.mjs';
...
const P = { decimals: 6, legacyCSS: false, spacesAfterCommas: false, anglesUnit: k.NONE, rgbUnit: w.NONE, labUnit: w.NONE, lchUnit: w.NONE, cmykUnit: w.PERCENT, alphaUnit: w.NONE, cmykFunction: I.DEVICE_CMYK }, O = (t2, e2) => Object.prototype.hasOwnProperty.call(t2, e2), x = (t2) => +`${t2}`.replace(S, "$1"), j = (t2) => S.test(`${t2}`) ? x(t2) : Math.min(+t2, 100), D = (t2) => (1 === t2.length && (t2 += t2), parseInt(t2, 16)), $ = (t2) => {
...
}, W = (t2, e2 = false) => !e2 && S.test(t2) ? Math.min(d * x(t2) / 100, d) : y.test(t2) ? (3 === t2.length && (t2 += t2.slice(-1)), e2 ? U(t2) / d : U(t2)) : Math.min(+t2, e2 ? 1 : d), N = (t2) => S.test(t2) ? K(m * x(t2) / 100, -125, m) : K(+t2, -125, m), T = (t2) => S.test(t2) ? K(A * x(t2) / 100, -150, A) : K(+t2, -150, A), X = (t2) => Math.min(S.test(t2) ? x(t2) / 100 : +t2, 1), Y = (t2) => [...t2].sort().join("").toUpperCase(), U = (t2, e2 = 6) => {
  const s2 = Math.pow(10, e2);
  return Math.round(+t2 * s2) / s2;
}, K = (t2, e2, s2) => Math.max(e2, Math.min(t2, s2)), V = (t2) => 180 * t2 / Math.PI, z = (t2) => t2 * Math.PI / 180, q = 360, Q = (t2) => {
  if ("string" == typeof t2) {
...
}, J = (t2) => ("string" == typeof t2 && (t2 = S.test(t2) ? x(t2) / 100 : +t2), isNaN(+t2) || t2 > 1 ? 1 : U(t2)), Z = (t2, e2) => {
  let s2;
  switch (e2) {
    case k.RADIANS:
      s2 = U(z(t2));
  ...
  t2 /= d, e2 /= d, s2 /= d, r2 = Math.min(r2, 1);
  ...
  const r2 = [t2 / d, e2 / d, s2 / d].map(it), a2 = ot(r2[0], r2[1], r2[2], et), i2 = ((t3, e3, s3) => {
  ...
  return { R: K(a2[0] * d, 0, d), G: K(a2[1] * d, 0, d), B: K(a2[2] * d, 0, d) };
}, ht = (t2, e2, s2) => {
  const r2 = Math.sqrt(e2 ** 2 + s2 ** 2), a2 = V(Math.atan2(s2, e2));
...
}, pt = (t2, e2, s2, r2 = 1) => ({ H: lt(t2, e2, s2, r2).H, W: U(Math.min(t2, e2, s2) / d * 100), B: U(100 * (1 - Math.max(t2, e2, s2) / d)), A: r2 }), gt = (t2, e2, s2) => {
  const r2 = 1 - (s2 /= 100) - (e2 /= 100), a2 = r2 * (1 - Math.abs(t2 / 60 % 2 - 1));
  let i2 = 0, n2 = 0, o2 = 0;
  return t2 < 60 ? (i2 = r2, n2 = a2, o2 = 0) : t2 >= 60 && t2 < 120 ? (i2 = a2, n2 = r2, o2 = 0) : t2 >= 120 && t2 < 180 ? (i2 = 0, n2 = r2, o2 = a2) : t2 >= 180 && t2 < 240 ? (i2 = 0, n2 = a2, o2 = r2) : t2 >= 240 && t2 < 300 ? (i2 = a2, n2 = 0, o2 = r2) : t2 >= 300 && (i2 = r2, n2 = 0, o2 = a2), i2 += e2, n2 += e2, o2 += e2, { R: K(i2 * d, 0, d), G: K(n2 * d, 0, d), B: K(o2 * d, 0, d) };
}, Ht = (t2, e2, s2, r2) => ({ R: U(d * (1 - t2) * (r2 = 1 - r2)), G: U(d * (1 - e2) * r2), B: U(d * (1 - s2) * r2) }), Ct = (t2, e2, s2) => {
  t2 /= d, e2 /= d, s2 /= d;
...
class Et extends Bt {
  constructor(t2) {
    ...
    void 0 !== this._a && (s2.A = D(this._a) / d), this._rgb = s2;
  }
}
class ft {
...
class Rt extends Ft {
  constructor(t2, e2) {
    var _a;
    super();
    const s2 = t2.match(F.RGB).groups, { r_legacy: r2, g_legacy: a2, b_legacy: i2, a_legacy: n2, r: o2, g: c2, b: l2, a: u2, from: h2, relative_r: m2, relative_g: A2, relative_b: b2, relative_a: p2 } = s2;
    if (h2) {
      const t3 = e2(h2), s3 = { r: t3.R, g: t3.G, b: t3.B, alpha: (_a = t3.A) != null ? _a : 1 }, r3 = new ft("r", m2, s3).result, a3 = new ft("g", A2, s3).result, i3 = new ft("b", b2, s3).result, n3 = { R: Math.min(r3, d), G: Math.min(a3, d), B: Math.min(i3, d) };
      if (p2) {
      ...
      const t3 = { R: Math.min(W(this._r), d), G: Math.min(W(this._g), d), B: Math.min(W(this._b), d) };
      void 0 !== this._a && (t3.A = J(this._a)), this._rgb = t3;
    }
  }
...
class St extends _t {
  constructor(t2, e2) {
    super();
    const s2 = t2.match(F.HSL).groups, { h_legacy: r2, s_legacy: a2, l_legacy: i2, a_legacy: n2, h: o2, s: c2, l: l2, a: u2, from: d2, relative_h: m2, relative_s: A2, relative_l: b2, relative_a: p2 } = s2;
    if (d2) {
      const t3 = e2(d2), s3 = lt(t3.R, t3.G, t3.B, t3.A), r3 = { h: s3.H, s: s3.S, l: s3.L, alpha: s3.A }, a3 = new ft("h", m2, r3).result, i3 = new ft("s", A2, r3).result, n3 = new ft("l", b2, r3).result, o3 = ct(K(a3, 0, 360), K(i3, 0, h), K(n3, 0, h));
      if (p2) {
...
class yt extends _t {
  constructor(t2, e2) {
    super();
    const s2 = t2.match(F.HWB).groups, { h: r2, w: a2, b: i2, a: n2, from: o2, relative_h: c2, relative_w: l2, relative_b: u2, relative_a: d2 } = s2;
    if (o2) {
      const t3 = e2(o2), s3 = pt(t3.R, t3.G, t3.B, t3.A), r3 = { h: s3.H, w: s3.W, b: s3.B, alpha: s3.A }, a3 = new ft("h", c2, r3).result, i3 = new ft("w", l2, r3).result, n3 = new ft("b", u2, r3).result, m2 = gt(K(a3, 0, 360), K(i3, 0, h), K(n3, 0, h));
      if (d2) {
        const t4 = new ft("alpha", d2, r3).result;
        m2.A = K(t4, 0, 1);
      }
      this._rgb = m2;
    } else {
...
class vt extends Ft {
  constructor(t2, e2) {
    var _a;
    super();
    const s2 = t2.match(F.CIELab).groups, { L: r2, a: a2, b: i2, A: n2, from: o2, relative_L: c2, relative_a: l2, relative_b: u2, relative_A: d2 } = s2;
    if (o2) {
      const t3 = e2(o2), s3 = ut(t3.R, t3.G, t3.B), r3 = { l: s3.L, a: s3.a, b: s3.b, alpha: (_a = t3.A) != null ? _a : 1 }, a3 = new ft("l", c2, r3).result, i3 = new ft("a", l2, r3).result, n3 = new ft("b", u2, r3).result, A2 = dt(K(a3, 0, h), K(i3, -125, m), K(n3, -125, m));
      if (d2) {
        const t4 = new ft("alpha", d2, r3).result;
        A2.A = K(t4, 0, 1);
      }
      ...
      const t3 = dt(j(this._L), N(this._A), N(this._B));
      void 0 !== this._a && (t3.A = J(this._a)), this._rgb = t3;
    }
  }
  get hasPercentageValues() {
...
class Gt extends _t {
  constructor(t2, e2) {
    var _a;
    super();
    const s2 = t2.match(F.LCH).groups, { l: r2, c: a2, h: i2, a: n2, from: o2, relative_l: c2, relative_c: l2, relative_h: u2, relative_a: d2 } = s2;
    if (o2) {
      const t3 = e2(o2), s3 = At(t3.R, t3.G, t3.B), r3 = { l: s3.L, c: s3.C, h: s3.H, alpha: (_a = t3.A) != null ? _a : 1 }, a3 = new ft("l", c2, r3).result, i3 = new ft("c", l2, r3).result, n3 = new ft("h", u2, r3).result, m2 = bt(K(a3, 0, h), K(i3, -150, A), Q(n3));
      if (d2) {
        const t4 = new ft("alpha", d2, r3).result;
        m2.A = K(t4, 0, 1);
      }
...
class Mt extends Ft {
  constructor(t2) {
    ...
    void 0 !== this._a && (s2.A = J(this._a)), this._rgb = s2;
  }
  get hasPercentageValues() {
...
const kt = (t2) => "string" == typeof t2 ? ((t3) => {
  ...
  if (E[r2] && (e2 = E[r2]), e2 && e2 === b.RGB) {
    const r3 = Object.entries(t3).some((t4) => !y.test(`${t4[1]}`)), a2 = Object.entries(t3).some((t4) => !(S.test(`${t4[1]}`) || !y.test(`${t4[1]}`) && !isNaN(+t4[1]) && +t4[1] <= d));
  ...
  return O(t2, "A") && (r2.A = J(t2.A)), r2;
}, [b.HWB](t2) {
  const e2 = j(`${t2.W}`), s2 = j(`${t2.B}`), r2 = gt(Q(t2.H), e2, s2);
  return O(t2, "A") && (r2.A = J(t2.A)), r2;
}, [b.CIELab](t2) {
  const e2 = j(`${t2.L}`), s2 = N(`${t2.a}`), r2 = N(`${t2.b}`), a2 = dt(e2, s2, r2);
  return O(t2, "A") && (a2.A = J(t2.A)), a2;
}, [b.LCH](t2) {
  const e2 = j(`${t2.L}`), s2 = T(`${t2.C}`), r2 = Q(`${t2.H}`), a2 = bt(e2, s2, r2);
  return O(t2, "A") && (a2.A = J(t2.A)), a2;
}, [b.CMYK](t2) {
  const e2 = X(`${t2.C}`), s2 = X(`${t2.M}`), r2 = X(`${t2.Y}`), a2 = X(`${t2.K}`), i2 = Ht(e2, s2, r2, a2);
  return O(t2, "A") && (i2.A = J(t2.A)), i2;
} }, Pt = (t2, e2 = kt(t2)) => "string" == typeof t2 ? wt[e2](t2) : It[e2](t2), Ot = (t2, ...e2) => {
  const s2 = [], r2 = [], a2 = [], i2 = [], n2 = [], o2 = [], c2 = [], l2 = Object.values(k), u2 = Object.values(w), d2 = Object.values(I), h2 = { legacyCSS: 0, spacesAfterCommas: 0, cmykFunction: 0 };
  for (const m2 of e2) if ("string" == typeof m2) {
    if (s2.push(m2), m2.includes(",")) {
      h2.legacyCSS++;
      const t3 = m2.match(G);
      1 === new Set(t3).size && M.test(t3[0].slice(1)) && h2.spacesAfterCommas++;
    }
    if (St.test(m2)) {
    ...
    if (Mt.test(m2)) {
      const t3 = new Mt(m2);
      o2.push(t3.hasPercentageValues), m2.startsWith("cmyk") && h2.cmykFunction++, c2.push(t3.hasPercentageAlpha);
    }
  }
  return { decimals: typeof t2.decimals === f.NUMBER ? t2.decimals : P.decimals, legacyCSS: typeof t2.legacyCSS === f.BOOLEAN ? t2.legacyCSS : Boolean(s2.length && h2.legacyCSS === s2.length) || P.legacyCSS, spacesAfterCommas: typeof t2.spacesAfterCommas === f.BOOLEAN ? t2.spacesAfterCommas : Boolean(s2.length && h2.spacesAfterCommas === s2.length) || P.spacesAfterCommas, anglesUnit: t2.anglesUnit && l2.includes(t2.anglesUnit) ? t2.anglesUnit : 1 === new Set(r2).size ? r2[0] : P.anglesUnit, rgbUnit: t2.rgbUnit && u2.includes(t2.rgbUnit) ? t2.rgbUnit : 1 === new Set(a2).size && a2[0] ? w.PERCENT : P.rgbUnit, labUnit: t2.labUnit && u2.includes(t2.labUnit) ? t2.labUnit : 1 === new Set(i2).size && i2[0] ? w.PERCENT : P.labUnit, lchUnit: t2.lchUnit && u2.includes(t2.lchUnit) ? t2.lchUnit : 1 === new Set(n2).size && n2[0] ? w.PERCENT : P.lchUnit, cmykUnit: t2.cmykUnit && u2.includes(t2.cmykUnit) ? t2.cmykUnit : 1 !== new Set(o2).size || o2[0] ? P.cmykUnit : w.NONE, alphaUnit: t2.alphaUnit && u2.includes(t2.alphaUnit) ? t2.alphaUnit : 1 === new Set(c2).size && c2[0] ? w.PERCENT : P.alphaUnit, cmykFunction: t2.cmykFunction && d2.includes(t2.cmykFunction) ? t2.cmykFunction : o2.length && o2.length === h2.cmykFunction ? I.CMYK : P.cmykFunction };
  ...
  return (r2, a2) => 0 === a2 && e2 !== k.NONE ? `${U(Z(r2, e2), s2)}${e2}` : 3 === a2 ? Wt(r2, t2) : U(r2, s2);
}, Tt = { [b.HEX]: (t2) => {
  ...
  const { decimals: s2, legacyCSS: r2, spacesAfterCommas: a2, rgbUnit: i2 } = e2, n2 = jt(a2), o2 = Dt(t2, (t3, r3) => {
    return i2 === w.PERCENT && r3 < 3 ? `${a3 = t3, n3 = s2, U(a3 / d * 100, n3)}%` : 3 === r3 ? Wt(t3, e2) : U(t3, s2);
    var a3, n3;
  ...
  const { decimals: s2, lchUnit: r2, anglesUnit: a2 } = e2, i2 = Dt(t2, (t3, i3) => {
    ...
    return 1 === i3 ? r2 === w.PERCENT ? `${n3 = t3, o2 = s2, U(n3 / A * 100, o2)}%` : U(t3, s2) : 2 === i3 ? a2 !== k.NONE ? `${U(Z(t3, a2), s2)}${a2}` : U(t3, s2) : Wt(t3, e2, true);
    var n3, o2;
  ...
  return $t(l2, c2);
} }, Xt = (t2, e2, s2) => e2.reduce((e3, r2) => [...e3, { ...t2, H: s2 === g.ADDITIVE ? Q(t2.H + r2) : Q(Lt(Lt(t2.H, false) + r2, true)) }], [{ ...t2 }]), Yt = (t2, e2) => Xt(t2, [30, -30], e2), Ut = (t2, e2) => Xt(t2, [180], e2), Kt = (t2, e2) => Xt(t2, [150, -150], e2), Vt = (t2, e2) => Xt(t2, [120, -120], e2), zt = (t2, e2) => Xt(t2, [60, -120, 180], e2), qt = (t2, e2) => Xt(t2, [90, -90, 180], e2), Qt = { [b.HEX]: (t2) => ({ R: $(t2.R), G: $(t2.G), B: $(t2.B) }), HEXA(t2) {
  const e2 = Qt.HEX(t2);
  return e2.A = O(t2, "A") ? $(t2.A * d) : "0xFF", e2;
}, [b.RGB](t2, e2) {
  const s2 = ee(t2, e2);
  ...
  switch (a2) {
    ...
    default:
      return u2.map((t3) => {
        ...
        return o2 && (e3.A = t3.A), i2 ? o2 ? Tt.HEX({ ...e3, A: U(e3.A * d) }) : Tt.HEX(e3) : o2 ? Qt.HEXA(e3) : Qt.HEX(e3);
      });
    case b.RGB:
      return u2.map((t3) => {
        const e3 = ct(t3.H, t3.S, t3.L);
...
}, [b.HEX]: (t2, e2, s2, r2) => e2(t2, s2).map((t3) => r2 ? Tt.HEX(ct(t3.H, t3.S, t3.L)) : Qt.HEX(ct(t3.H, t3.S, t3.L))), HEXA: (t2, e2, s2, r2) => e2(t2, s2).map((t3) => r2 ? Tt.HEX({ ...ct(t3.H, t3.S, t3.L), A: J(t3.A) * d }) : Qt.HEXA({ ...ct(t3.H, t3.S, t3.L), A: J(t3.A) })), [b.RGB]: (t2, e2, s2, r2, a2) => e2(t2, s2).map((t3) => r2 ? Tt.RGB(ct(t3.H, t3.S, t3.L), a2) : Qt.RGB(ct(t3.H, t3.S, t3.L), a2.decimals)), RGBA: (t2, e2, s2, r2, a2) => e2(t2, s2).map((t3) => r2 ? Tt.RGB({ ...ct(t3.H, t3.S, t3.L), A: J(t3.A) }, a2) : Qt.RGBA({ ...ct(t3.H, t3.S, t3.L), A: J(t3.A) }, a2.decimals)), [b.HSL]: (t2, e2, s2, r2, a2) => e2(t2, s2).map((t3) => r2 ? Tt.HSL({ H: t3.H, S: t3.S, L: t3.L }, a2) : Qt.HSL(ct(t3.H, t3.S, t3.L), a2.decimals)), HSLA: (t2, e2, s2, r2, a2) => e2(t2, s2).map((t3) => r2 ? Tt.HSL({ ...t3, A: J(t3.A) }, a2) : Qt.HSLA({ ...ct(t3.H, t3.S, t3.L), A: J(t3.A) }, a2.decimals)), [b.HWB]: (t2, e2, s2, r2, a2) => e2(t2, s2).map((t3) => {
  ...
  const e3 = ct(t3.H, t3.S, t3.L), s3 = pt(e3.R, e3.G, e3.B);
  return r2 ? Tt.HWB({ ...s3, A: J(t3.A) }, a2) : Qt.HWBA({ ...e3, A: J(t3.A) }, a2.decimals);
  ...
  const e3 = ct(t3.H, t3.S, t3.L);
  return r2 ? Tt.CIELab({ ...ut(e3.R, e3.G, e3.B), A: J(t3.A) }, a2) : Qt.CIELabA({ ...e3, A: J(t3.A) }, a2.decimals);
  ...
  const e3 = ct(t3.H, t3.S, t3.L);
  return r2 ? Tt.LCH({ ...At(e3.R, e3.G, e3.B), A: J(t3.A) }, a2) : Qt.LCHA({ ...e3, A: J(t3.A) }, a2.decimals);
}) }, te = { mix(t2, e2) {
  const s2 = t2.map((t3) => {
    const e3 = kt(t3);
    return Pt(t3, e3);
    ...
    const e3 = ((t4, e4, s3) => {
      const r3 = Math.min(t4, e4, s3), a3 = Math.min(d - t4, d - e4, d - s3), i3 = t4 - r3, n2 = e4 - r3, o2 = s3 - r3, c2 = Math.min(i3, n2), l2 = i3 - c2, u2 = (n2 + c2) / 2, h2 = (o2 + n2 - c2) / 2, m2 = Math.max(l2, u2, h2) / Math.max(i3, n2, o2), A2 = isNaN(m2) || m2 === 1 / 0 || m2 <= 0 ? 1 : m2;
      return { R: l2 / A2 + a3, Y: u2 / A2 + a3, B: h2 / A2 + a3 };
    })(t3.R, t3.G, t3.B);
  ...
  function a2(t3) {
    ...
    return t3.reduce((t4, s4) => {
      const r3 = O(s4, "A") ? s4.A : 1, a3 = { R: Math.min(t4.R + s4.R * r3, d), B: Math.min(t4.B + s4.B * r3, d), A: 1 - (1 - r3) * (1 - t4.A) }, i3 = "G" in t4 ? t4.G : t4.Y, n2 = "G" in s4 ? s4.G : s4.Y;
      return { ...a3, ...e2 === g.ADDITIVE ? { G: Math.min(i3 + n2 * r3, d) } : { Y: Math.min(i3 + n2 * r3, d) } };
    }, s3);
  }
  let i2;
  if (e2 === g.ADDITIVE) i2 = a2(s2);
    ...
    const t3 = a2(r2);
    i2 = ((t4, e3, s3) => {
      const r3 = Math.min(t4, e3, s3), a3 = Math.min(d - t4, d - e3, d - s3), i3 = t4 - r3, n2 = e3 - r3, o2 = s3 - r3, c2 = Math.min(n2, o2), l2 = i3 + n2 - c2, u2 = n2 + c2, h2 = 2 * (o2 - c2), m2 = Math.max(l2, u2, h2) / Math.max(i3, n2, o2), A2 = isNaN(m2) || m2 === 1 / 0 || m2 <= 0 ? 1 : m2;
      return { R: l2 / A2 + a3, G: u2 / A2 + a3, B: h2 / A2 + a3 };
    })(t3.R, t3.Y, t3.B), i2.A = t3.A;
  }
  return { R: U(i2.R), G: U(i2.G), B: U(i2.B), A: K(i2.A, 0, 1) };
}, [b.HEX](t2, e2, s2) {
  const r2 = this.mix(t2, e2);
  return delete r2.A, s2 ? Tt.HEX(r2) : Qt.HEX(r2);
}, HEXA(t2, e2, s2) {
  const r2 = this.mix(t2, e2);
  return r2.A = s2 ? J(r2.A) * d : J(r2.A), s2 ? Tt.HEX(r2) : Qt.HEXA(r2);
}, [b.RGB](t2, e2, s2, r2) {
  ...
  s2 < 1 && (s2 = 5);
  return ((t3, e3, s3) => {
    const r3 = s3 - 1, a3 = (e3.R - t3.R) / r3, i2 = (e3.G - t3.G) / r3, n2 = (e3.B - t3.B) / r3, o2 = J(t3.A), c2 = (J(e3.A) - o2) / r3;
...
const ne = (t2, e2, s2, r2) => ({ [p.ANALOGOUS]: Zt.buildHarmony(e2, Yt, s2, r2), [p.COMPLEMENTARY]: Zt.buildHarmony(e2, Ut, s2, r2), [p.SPLIT_COMPLEMENTARY]: Zt.buildHarmony(e2, Kt, s2, r2), [p.TRIADIC]: Zt.buildHarmony(e2, Vt, s2, r2), [p.TETRADIC]: Zt.buildHarmony(e2, zt, s2, r2), [p.SQUARE]: Zt.buildHarmony(e2, qt, s2, r2) })[t2], oe = Object.fromEntries(Object.entries(te).map((t2) => {
  const [e2, s2] = t2;
...
class ce {
  ...
  updateRGBFromLAB() {
    this.rgb = { ...dt(this.lab.L, this.lab.a, this.lab.b), A: this.rgb.A };
  }
  updateLCHFromLAB() {
    this.lch = { ...ht(this.lab.L, this.lab.a, this.lab.b), A: this.lab.A };
  }
  updateLABromLCH() {
  ...
  setR(t2) {
    this.rgb.R = K(t2, 0, d), this.update("rgb");
  }
  setG(t2) {
    return this.rgb.G = K(t2, 0, d), this.update("rgb"), this;
  }
  setB(t2) {
    return this.rgb.B = K(t2, 0, d), this.update("rgb"), this;
  }
  setH(t2) {
    return this.hsl.H = Q(t2), this.update("hsl"), this;
  }
  setS(t2) {
    return this.hsl.S = K(t2, 0, h), this.update("hsl"), this;
  }
  setL(t2) {
    return this.hsl.L = K(t2, 0, h), this.update("hsl"), this;
  }
  setWhiteness(t2) {
    return this.hwb.W = K(t2, 0, h), this.updateRGBFromHWB(), this.update("rgb", "hwb"), this;
  }
  setBlackness(t2) {
    return this.hwb.B = K(t2, 0, h), this.updateRGBFromHWB(), this.update("rgb", "hwb"), this;
  }
  setCIEL(t2) {
    return this.lab.L = K(t2, 0, h), this.updateRGBFromLAB(), this.updateLCHFromLAB(), this.update("rgb", "lab", "lch"), this;
  }
  setCIEa(t2) {
    return this.lab.a = K(t2, -125, m), this.updateRGBFromLAB(), this.updateLCHFromLAB(), this.update("rgb", "lab", "lch"), this;
  }
  setCIEb(t2) {
    return this.lab.b = K(t2, -125, m), this.updateRGBFromLAB(), this.updateLCHFromLAB(), this.update("rgb", "lab", "lch"), this;
  }
  setLCHL(t2) {
    return this.lch.L = K(t2, 0, h), this.updateRGBFromLCH(), this.updateLABromLCH(), this.update("rgb", "lab", "lch"), this;
  }
  setLCHC(t2) {
    return this.lch.C = K(t2, 0, A), this.updateRGBFromLCH(), this.updateLABromLCH(), this.update("rgb", "lab", "lch"), this;
  }
  setLCHH(t2) {
  ...
  setC(t2) {
    return this.cmyk.C = K(t2, 0, 100), this.updateRGBFromCMYK(), this.update("cmyk"), this;
  }
  setM(t2) {
    return this.cmyk.M = K(t2, 0, 100), this.updateRGBFromCMYK(), this.update("cmyk"), this;
  }
  setY(t2) {
    return this.cmyk.Y = K(t2, 0, 100), this.updateRGBFromCMYK(), this.update("cmyk"), this;
  }
  setK(t2) {
    return this.cmyk.K = K(t2, 0, 100), this.updateRGBFromCMYK(), this.update("cmyk"), this;
  }
  get options() {
  ...
  get HEXA() {
    return Tt.HEX({ R: this.R, G: this.G, B: this.B, A: this.A * d });
  }
  get RGB() {
  ...
  static getShades(t2, e2, s2) {
    return "number" == typeof e2 ? Jt(t2, e2, true, Ot(s2 || {}, t2)) : Jt(t2, 5, true, Ot(e2 || {}, t2));
  }
  static getTints(t2, e2, s2) {
    return "number" == typeof e2 ? Jt(t2, e2, false, Ot(s2 || {}, t2)) : Jt(t2, 5, false, Ot(e2 || {}, t2));
  }
...
const de = { __name: "ColorPicker", __ssrInlineRender: true, props: Wt$1.mergeModels({ as: { type: null, required: false }, throttle: { type: Number, required: false, default: 50 }, disabled: { type: Boolean, required: false }, defaultValue: { type: String, required: false, default: "#FFFFFF" }, format: { type: String, required: false, default: "hex" }, size: { type: null, required: false }, class: { type: null, required: false }, ui: { type: null, required: false } }, { modelValue: { type: String }, modelModifiers: {} }), emits: ["update:modelValue"], setup(t2) {
  const c2 = t2, u2 = Wt$1.useModel(t2, "modelValue", { type: String }), d2 = da(), h2 = Wt$1.computed(() => {
    var t3;
    return Ky({ extend: Ky(le), ...(null == (t3 = d2.ui) ? void 0 : t3.colorPicker) || {} })({ size: c2.size });
...
const me = o(de, [["__scopeId", "data-v-2f14ca51"]]), Ae = Wt$1.defineComponent({ __name: "ColorPickerField", __ssrInlineRender: true, props: { modelValue: { type: String, default: "#000000" }, format: { type: String, default: "hex" }, size: { type: String, default: "md" }, disabled: { type: Boolean, default: false }, as: { type: String, default: "div" }, ui: { type: Object, default: () => ({}) }, label: { type: String, default: "Select Color" } }, emits: ["update:modelValue"], setup(s2, { emit: r2 }) {
  ...
  return (s3, r3, u2, d2) => {
    const h2 = i, m2 = Rb, A2 = me;
    r3(pm.ssrRenderComponent(h2, d2, { content: Wt$1.withCtx((t2, s4, r4, o3) => {
...
//# sourceMappingURL=ColorPickerField-qbnRmFc_.mjs.map
