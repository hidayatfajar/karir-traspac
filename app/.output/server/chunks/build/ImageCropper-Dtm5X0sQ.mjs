...
function ut(t2) {
  ...
  var g2 = it(d2 = Z(d2, u2.width * i2.width / (d2.width * p2.width)), e2({ visibleArea: d2, type: "resize" }));
  return 1 !== g2 && (d2 = Z(d2, g2), u2 = Z(u2, g2)), d2 = nt(d2 = _(d2, Y(X(u2), X(d2))), e2({ visibleArea: d2, type: "move" })), { coordinates: u2 = nt(u2, rt(N(d2), a2)), visibleArea: d2 };
}
function dt(t2) {
...
function wt(t2) {
  ...
  if (c2 = U(i2, h2).width, l2 = U(i2, h2).height, h2.right + h2.left && (c2 > s2.maxWidth ? d2.width = (s2.maxWidth - i2.width) / (h2.right + h2.left) : c2 < s2.minWidth && (d2.width = (s2.minWidth - i2.width) / (h2.right + h2.left))), h2.bottom + h2.top && (l2 > s2.maxHeight ? d2.height = (s2.maxHeight - i2.height) / (h2.bottom + h2.top) : l2 < s2.minHeight && (d2.height = (s2.minHeight - i2.height) / (h2.bottom + h2.top))), r2) {
    var g2 = Math.min(d2.width, d2.height);
    g2 !== 1 / 0 && O.forEach(function(t3) {
      h2[t3] *= g2;
    });
...
var Ht = { props: { touchMove: { type: Boolean, required: true }, mouseMove: { type: Boolean, required: true }, touchResize: { type: Boolean, required: true }, wheelResize: { type: [Boolean, Object], required: true }, eventsFilter: { type: Function, required: false } }, beforeMount: function() {
  ...
  if (d2) {
    ...
    if (p2 || (p2 = r2.width >= r2.height || 1 === d2 ? "width" : "height"), "width" === p2) {
      var g2 = l2 / d2 - r2.height;
      if (c2.top && c2.bottom) {
        var f2 = h2.top, v2 = h2.bottom;
        h2.bottom = yt(g2, v2, f2), h2.top = yt(g2, f2, v2);
      } else c2.bottom ? h2.bottom = g2 : c2.top ? h2.top = g2 : c2.right ? h2.right = 0 : c2.left && (h2.left = 0);
    } else if ("height" === p2) {
  ...
  this.imageAttributes.revoke && this.imageAttributes.src && URL.revokeObjectURL(this.imageAttributes.src), this.imageAttributes.revoke = false, n2 && s2 && s2 > 1 ? g(i2) || !f(i2) ? (this.imageAttributes.src = URL.createObjectURL(new Blob([n2])), this.imageAttributes.revoke = true) : this.imageAttributes.src = function(t3) {
  ...
  if (!this.transitionsOptions.enabled) {
    ...
    var a2 = function(t3) {
      var e3 = t3.event, i3 = t3.coordinates, n3 = t3.visibleArea, o2 = t3.sizeRestrictions, s3 = t3.getAreaRestrictions, r3 = t3.positionRestrictions, a3 = t3.adjustStencil, h3 = e3.scale, c3 = e3.move, l2 = m({}, n3), u2 = m({}, i3), d2 = 1, p2 = 1, g2 = h3.factor && Math.abs(h3.factor - 1) > 1e-3;
      l2 = _(l2, { left: c3.left || 0, top: c3.top || 0 });
      var f2 = { stencil: { minimum: Math.max(o2.minWidth ? o2.minWidth / u2.width : 0, o2.minHeight ? o2.minHeight / u2.height : 0), maximum: Math.min(o2.maxWidth ? o2.maxWidth / u2.width : 1 / 0, o2.maxHeight ? o2.maxHeight / u2.height : 1 / 0, G(u2, r3)) }, area: { maximum: G(l2, s3({ visibleArea: l2, type: "resize" })) } };
      h3.factor && g2 && (h3.factor < 1 ? (p2 = Math.max(h3.factor, f2.stencil.minimum)) > 1 && (p2 = 1) : h3.factor > 1 && (p2 = Math.min(h3.factor, Math.min(f2.area.maximum, f2.stencil.maximum))) < 1 && (p2 = 1)), p2 && (l2 = Z(l2, p2, h3.center));
      var v2 = i3.left - n3.left, b2 = n3.width + n3.left - (i3.width + i3.left), w2 = i3.top - n3.top, y2 = n3.height + n3.top - (i3.height + i3.top);
      return l2 = nt(l2 = _(l2, Q(l2, { left: void 0 !== r3.left ? r3.left - v2 * p2 : void 0, top: void 0 !== r3.top ? r3.top - w2 * p2 : void 0, bottom: void 0 !== r3.bottom ? r3.bottom + y2 * p2 : void 0, right: void 0 !== r3.right ? r3.right + b2 * p2 : void 0 })), s3({ visibleArea: l2, type: "move" })), u2.width = u2.width * p2, u2.height = u2.height * p2, u2.left = l2.left + v2 * p2, u2.top = l2.top + w2 * p2, u2 = nt(u2, rt(N(l2), r3)), h3.factor && g2 && a3 && (h3.factor > 1 ? d2 = Math.min(f2.area.maximum, h3.factor) / p2 : h3.factor < 1 && (d2 = Math.max(u2.height / l2.height, u2.width / l2.width, h3.factor / p2)), 1 !== d2 && (l2 = _(l2 = nt(l2 = Z(l2, d2, h3.factor > 1 ? h3.center : X(u2)), s3({ visibleArea: l2, type: "move" })), V(Q(u2, N(l2)))))), { coordinates: u2, visibleArea: l2 };
  ...
  if (!this.transitionsActive) {
    ...
    var s2 = o({}, this.imageTransforms.flip), r2 = function(t3) {
      ...
      if (l2 || u2) {
        var d2 = et({ left: a3.width / 2, top: a3.height / 2 }, -n3), p2 = et(X(h3), -n3), g2 = et({ left: l2 ? d2.left - (p2.left - d2.left) : p2.left, top: u2 ? d2.top - (p2.top - d2.top) : p2.top }, n3);
        h3 = _(h3, Y(g2, X(h3))), p2 = et(X(c2), -n3), c2 = nt(c2 = _(c2, Y(g2 = et({ left: l2 ? d2.left - (p2.left - d2.left) : p2.left, top: u2 ? d2.top - (p2.top - d2.top) : p2.top }, n3), X(c2))), o2({ visibleArea: c2, type: "move" }));
      }
...
//# sourceMappingURL=ImageCropper-Dtm5X0sQ.mjs.map
